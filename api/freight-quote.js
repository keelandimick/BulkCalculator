...
function getStateFromZip(zip) {
    if (!zip) return null;
    const prefix = zip.toString().padStart(5, '0').substring(0, 3);
...
async function getAccessToken() {
    ...
    console.log('Client ID (first 10 chars):', UBER_FREIGHT_CLIENT_ID ? UBER_FREIGHT_CLIENT_ID.substring(0, 10) + '...' : 'NOT SET');
    console.log('Client Secret set:', UBER_FREIGHT_CLIENT_SECRET ? 'YES' : 'NO');
...
module.exports = async function handler(req, res) {
    ...
    try {
        ...
        // Get state codes from zip codes
        const originState = getStateFromZip(origin_zip);
        const destState = getStateFromZip(destination_zip);
        ...
        if (!originState || !destState) {
            return res.status(400).json({
                ...
                message: `Could not determine state for zip code: ${!originState ? origin_zip : destination_zip}`
            });
        }
        ...
        // Official UberFreight v2 API format with proper state codes
        const quoteRequest = {
            ...
            stops: [
                {
                    ...
                    facility: {
                        ...
                        address: {
                            line1: "123 Warehouse Dr",
                            city: "Warehouse City",
                            principal_subdivision: originState,
                ...
                },
                {
                    ...
                    facility: {
                        ...
                        address: {
                            ...
                            city: "Delivery City",
                            principal_subdivision: destState,
        ...
        if (quotes.length === 0) {
            return res.status(200).json({
                ...
                message: 'UberFreight returned no carrier quotes for this lane'
            });
        }

        // Format rates - amounts are in cents, convert to dollars
        const formattedRates = quotes.map(quote => ({
            carrier: quote.carrier_name || 'Uber Freight Network',
            service: quote.service_level || 'Standard LTL',
            total: (quote.rate_details?.all_in_rate?.amount / 100).toFixed(2),
            currency: quote.rate_details?.all_in_rate?.currency_code || 'USD',
            transit_days: quote.transit_estimate?.duration?.value || 'N/A',
            scac: quote.carrier_scac || 'UBER',
            quote_id: quote.uber_quote_id || uniqueQuoteId
        }));
        ...
        return res.status(200).json({
            ...
            quote_id: uniqueQuoteId,
            carrier_count: formattedRates.length
        });
